...
WEATHER_SERVER_PATH = BASE_DIR / "mcp_servers" / "custom_weather_mcp_server.py"
...
llm = ChatGroq(
    ...
    api_key=GROQ_API_KEY or "gsk_placeholder_key_for_testing_and_import",
)
...
"""
    ...
    try:
        ...
        if not destination or "gsk_placeholder" in str(response.content):
            words = [w.strip(".,!?") for w in query.split() if w.strip(".,!?")]
            return words[-1].title() if words else "Paris"
