...
from mcp_servers.custom_weather_mcp_server import app as weather_mcp_app
from tools.flight_tool import fetch_flight_data
from tools.tavily_tool import search_web_info
from agents.supervisor import supervisor_agent
from agents.guardrails import guardrail_blocked_agent
from agents.budget_agent import budget_agent


def get_database_url() -> str | None:
    raw_url = os.getenv("DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/travel_db")
    if not raw_url:
        return None
...
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "gsk_placeholder_key_for_testing_and_import")
...
def flight_agent(state: TravelState):
    ...
    try:
        flight_info = fetch_flight_data(query)
        airports = flight_info.get("airports", "") if isinstance(flight_info, dict) else str(flight_info)
        airlines = flight_info.get("airlines", "") if isinstance(flight_info, dict) else ""
...
def hotel_agent(state: TravelState):
    ...
    try:
        hotel_results = search_web_info(query)

    except Exception as exc:
        print(
            f"HOTEL AGENT TOOL ERROR: "
...
def init_checkpointer():
    ...
    if db_url:
        try:
            ...
            print("PostgreSQL State Checkpointer initialized successfully.")
            return postgres_checkpointer
        except Exception:
            print("[INFO] PostgreSQL connection unavailable. Initialized MemorySaver checkpointer.")
    else:
        print("[INFO] DATABASE_URL not set. Initialized MemorySaver checkpointer.")
